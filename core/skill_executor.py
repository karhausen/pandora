...
class SkillExecutor:
    def __init__(self, skill_registry: SkillRegistry, tool_registry: ToolRegistry, runtime_db: ToolRuntimeDB | None = None, episodic_memory: EpisodicMemory | None = None, reflection: ReflectionEngine | None = None, quality_db: SkillQualityDB | None = None):
    ...
    def _build_payload(self, original_input: dict, context: dict, input_map: dict[str, str], static_input: dict) -> dict:
        ...
        for target_key, source_path in input_map.items(): payload[target_key]=self._resolve_path(source, source_path)
        return payload
    async def run_skill(self, skill_id: str, payload: dict, timeout_per_step: float = 5.0, task: str | None = None) -> SkillResult:
        ...
        missing=[tid for tid in skill.required_tools if not self.tool_registry.get(tid)]
        if missing: return SkillResult(success=False, skill=skill_id, error=f"Missing required tools: {missing}")
        start=time.perf_counter(); context={}; step_results=[]; used_tools=[]
        for step in skill.steps:
            if step.type!="tool" or not step.tool_id:
                err=f"Invalid step {step.id}"; elapsed=time.perf_counter()-start; self._record(skill_id,False,elapsed,err,used_tools,task); return SkillResult(success=False,skill=skill_id,error=err,steps=step_results)
            ...
            used_tools.append(step.tool_id); step_results.append({"step_id":step.id,"tool_id":step.tool_id,"success":result.success,"output":result.output,"error":result.error,"execution_time":result.execution_time})
            if not result.success:
                elapsed=time.perf_counter()-start; self._record(skill_id,False,elapsed,result.error,used_tools,task); return SkillResult(success=False,skill=skill_id,error=result.error,steps=step_results,execution_time=elapsed)
            if step.save_as: context[step.save_as]=result.output
        elapsed=time.perf_counter()-start; self._record(skill_id,True,elapsed,None,used_tools,task); return SkillResult(success=True,skill=skill_id,output=context,steps=step_results,execution_time=elapsed)
    def _record(self, skill_id, success, elapsed, error, used_tools, task):
        ...
        self.episodic_memory.record(task=task or f"run-skill:{skill_id}", kind="skill", success=success, used_tools=used_tools, used_skills=[skill_id], execution_time=elapsed, error=error, summary=f"Skill {skill_id} {'completed successfully' if success else 'failed'}.")
