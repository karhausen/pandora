...
class EpisodicMemory:
        ...
        with self._connect() as con:
            con.execute(
                ...
                INSERT INTO episodes(
                    id, task, kind, success, used_tools, used_skills,
                    execution_time, error, summary, created_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    ...
                    episode.kind,
                    int(episode.success),
                    json.dumps(episode.used_tools),
                    json.dumps(episode.used_skills),
    ...
    def successful_tool_sequences(self, min_length: int = 2, limit: int = 200) -> list[list[str]]:
        episodes = self.list_recent(limit)
        sequences = []
        for ep in episodes:
            if ep.success and len(ep.used_tools) >= min_length:
                sequences.append(ep.used_tools)
        return sequences
    ...
    def _row_to_episode(self, row) -> Episode:
        return Episode(
            ...
            used_skills=json.loads(row["used_skills"]),
