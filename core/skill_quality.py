...
class SkillQualityDB:
    ...
    def get(self, skill_id: str) -> dict | None:
        with self._connect() as con:
            con.row_factory = sqlite3.Row; row = con.execute("SELECT * FROM skill_quality WHERE skill_id=?", (skill_id,)).fetchone(); return dict(row) if row else None
    def list(self) -> list[dict]:
        with self._connect() as con:
            con.row_factory = sqlite3.Row; return [dict(r) for r in con.execute("SELECT * FROM skill_quality ORDER BY score DESC").fetchall()]
