...
class ToolExecutor:
    def __init__(self, registry: ToolRegistry, runtime_db: ToolRuntimeDB | None = None, episodic_memory: EpisodicMemory | None = None, reflection: ReflectionEngine | None = None):
    ...
    async def run_tool(self, tool_id: str, payload: dict, timeout: float = 5.0, task: str | None = None) -> ToolResult:
        ...
        try:
            ...
            self.episodic_memory.record(task=task or f"run-tool:{tool_id}", kind="tool", success=True, used_tools=[tool_id], execution_time=elapsed, summary=f"Tool {tool_id} completed successfully.")
            ...
            self.episodic_memory.record(task=task or f"run-tool:{tool_id}", kind="tool", success=False, used_tools=[tool_id], execution_time=elapsed, error=err, summary=f"Tool {tool_id} failed.")
