...
class CloudToolCodeGenerator:
    ...
    def _generate_design_driven_code(self, design: ToolDesign) -> str:
        """Capability-agnostic fallback scaffold.

        This method intentionally does not implement domain-specific behavior.
        Real behavior must be generated by an LLM from ToolDesign. The fallback
        exists for mock/test plumbing and contract safety only.
        """
        output_lines = []
        for key, type_name in (design.output_schema or {"result": "str"}).items():
            output_lines.append(f"        {str(key)!r}: _default_value({str(type_name)!r}),")
        output_block = "\n".join(output_lines)
        return f'''TOOL_META = {{
    "id": "{design.tool_id}",
    "name": "{design.name}",
...
'''
    ...
    def _generate_design_driven_test(self, design: ToolDesign) -> str:
        ...
        keys = list((design.output_schema or {"result": "str"}).keys())
